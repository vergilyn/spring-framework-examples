    ...
    public boolean isLock(long timeout){
        ...
        try {
            while (!this.lock){  // FIXME: 会一直等到获取锁, 感觉可以引入获取锁超时机制, 如lock()的逻辑
                ...
                lockVal = getRedisTime() + TimeUnit.SECONDS.toMillis(timeout);
                // 1. 尝试获取锁
                 ...
                 */
                if(curTime > Long.valueOf(expireVal)
                        && curTime > Long.valueOf(this.redisClient.opsForValue().getAndSet(this.key , lockVal + ""))){
